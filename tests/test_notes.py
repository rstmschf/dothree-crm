...
class TestNoteAPI:
    ...
    def test_create_ai_note_triggers_celery(
        self, auth_api_client, setup_deal_data, user
    ):
        url = reverse("note-list")
        payload = {
            "deal": setup_deal_data.id,
        ...
        with (
            patch("sales.tasks.process_ai_note_task.delay") as mock_ai_task,
            patch("sales.tasks.broadcast_new_note_task.delay") as mock_ws_task,
        ):
            response = auth_api_client.post(url, payload, format="json")

            assert response.status_code == status.HTTP_201_CREATED
            assert "Processing" in response.data["text"]
            assert response.data["original_text"] == "summarize this deal"
    ...
    def test_cannot_add_note_to_others_deal(
        self, api_client, setup_deal_data, django_user_model
    ):
        ...
        payload = {"deal": setup_deal_data.id, "text": "Sneaking into your deal."}
